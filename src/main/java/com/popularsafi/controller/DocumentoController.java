...
public class DocumentoController {
    ...
    public ResponseEntity<JwtResponseRuta> enviarCorreo(@RequestBody JwtRequestRuta jwtRequestRuta) throws Exception {
        ...
        if (jwtRequestRuta.getCorreo() != null) {
            System.out.println("paso por acas2");
          ...
          //          "<p><p><p><p><img src='https://ci3.googleusercontent.com/mail-sig/AIorK4zp0xnuCxJiBiWM8IhxrixsCRYNscTIa2gACv65i5giCUA-Kq0YJFc9hyfFktNgdyFY22gifu0'  width='450' height='160'/> </html>";
            ...
            if (fDirectorio.exists()) {
                ...
                {
                    archivo=  ficheros[x].toString().trim();
                    System.out.println("este es  el archivo"+archivo);
                    if (archivo.contains(jwtRequestRuta.getNombredocumento().trim())){
                        mensaje="se envio el correo, Correctamente";
                        lnexiste=true;
                        file[0]=archivo;
                        System.out.println("paso por acas4");
                        iConsultaService.sendFromGMail(getUsuario(), getPassword(), to, subject, body, file);
                    }
                }
                if (lnexiste==false){
                    mensaje="no cuentas con dicho documento";
                    System.out.println("paso por acasno no existe archivo");
                }
            }else{
                System.out.println("paso por acasno existe");
                mensaje="no existe carpeta";
            }
        }else{
